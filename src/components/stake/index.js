...
const Stake = () => {
  ...
  return (
    <div id="stake" className="h-screen max-h-screen overflow-y-autobg-black justify-center snap-start">
      <div className="flex flex-col flex-1 h-full items-center justify-center">
        <div className="flex flex-1 flex-col md:flex-row justify-center items-center relative z-10 space-y-8 md:space-y-0 space-x-0 md:space-x-12">
          <div ref={textRef} className="font-saira text-white px-2 md:px-8 text-center md:text-left font-medium">
            <h1 className={`text-4xl sm:text-5xl mb-2 opacity-0 ${textVisible && "animate-fadeIn"}`}>Stake LP tokens.</h1>
            <h1 className={`text-4xl sm:text-5xl mb-2 opacity-0 ${textVisible && "animate-fadeIn"}`}>Claim every second.</h1>
            <div
